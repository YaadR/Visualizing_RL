...
class SnakeGameAI:
    ...
    def copy(self, copied_game):
        ...
        excluded_attributes = [
            "clock"
        ]  # Add any other attribute names you want to exclude
    ...
    def arrow_ui(self):
        if np.any(self.probability_clock):
            self.probability_clock = (
                self.probability_clock - np.min(self.probability_clock)
        ...
        if self.direction == Direction.RIGHT or Direction.UP or Direction.DOWN:
            arrow_points = [
                (
                    self.head.x + ahead,
                    self.head.y
                    - self.probability_clock[Direction.RIGHT.value - 1] // ratio
                    + correction,
                ),
                (
                    self.head.x
                    + ahead
                    + self.probability_clock[Direction.RIGHT.value - 1],
            ...
            color = (
                WHITE if self.probability_clock[Direction.RIGHT.value - 1] else BLACK
            )
        ...
        if self.direction == Direction.LEFT or Direction.UP or Direction.DOWN:
            arrow_points = [
                (
                    self.head.x - ahead // 2,
                    self.head.y
                    - self.probability_clock[Direction.LEFT.value] // ratio
                    + correction,
                ),
                (
                    self.head.x
                    - ahead // 2
                    - self.probability_clock[Direction.LEFT.value],
                    self.head.y + correction,
                ),
                (
                    self.head.x - ahead // 2,
        ...
        if self.direction == Direction.UP or Direction.LEFT or Direction.RIGHT:
            arrow_points = [
                (
                    self.head.x
                    + correction
                    - self.probability_clock[Direction.UP.value] // ratio,
                    self.head.y - ahead // 2,
                ),
                (
                    self.head.x + correction,
                    self.head.y
                    - self.probability_clock[Direction.UP.value]
                    - ahead // 2,
                ),
                (
                    self.head.x
        ...
        if self.direction == Direction.DOWN or Direction.LEFT or Direction.RIGHT:
            arrow_points = [
                (
                    self.head.x
                    + correction
                    - self.probability_clock[Direction.DOWN.value - 3] // ratio,
                    self.head.y + ahead,
                ),
                (
                    self.head.x + correction,
                    self.head.y
                    + self.probability_clock[Direction.DOWN.value - 3]
                    + ahead,
                ),
                (
                    self.head.x
    ...
    def _update_ui(self):
        ...
        if self.certainty_flag:
            ...
            # Draw percentage bar
            pygame.draw.rect(
                self.display, color, (10, 10, int(self.certainty * 120), 30), 0
            )  # Percentage bar
            pygame.draw.rect(
                self.display, color, (10, 10, 120, 30), 3
            )  # Outline of bar
        ...
        if self.agentID:
            text = font.render(
                AGENT_NAMES[self.agentID - 1] + " Score:" + str(self.score),
                True,
