...
class SnakeGameAICompetition:
    ...
    def play_step(self, action, agentID=0):
        ...
        reward = 0
        game_over = False
        if self.is_collision(agentID=agentID) or self.frame_iteration[
            agentID
        ] > 50 * len(self.snake[agentID]):
            game_over = True
            reward = -10
    ...
    def arrow_ui(self, agentID=0):
        if np.any(self.probability_clock[agentID]):
            self.probability_clock[agentID] = (
                self.probability_clock[agentID]
                - np.min(self.probability_clock[agentID])
            ) / (
                np.max(self.probability_clock[agentID])
                - np.min(self.probability_clock[agentID])
            )
        ...
        if self.direction[agentID] == Direction.RIGHT or Direction.UP or Direction.DOWN:
            arrow_points = [
                (
                    ...
                    self.head[agentID].y
                    - self.probability_clock[agentID][Direction.RIGHT.value - 1]
                    // ratio
                    + correction,
                ),
                (
            ...
            color = (
                WHITE
                if self.probability_clock[agentID][Direction.RIGHT.value - 1]
                else BLACK
            )
        ...
        if self.direction[agentID] == Direction.LEFT or Direction.UP or Direction.DOWN:
            arrow_points = [
                (
                    ...
                    self.head[agentID].y
                    - self.probability_clock[agentID][Direction.LEFT.value] // ratio
                    + correction,
                ),
                (
                    self.head[agentID].x
                    - ahead // 2
                    - self.probability_clock[agentID][Direction.LEFT.value],
                    self.head[agentID].y + correction,
                ),
                (
                    self.head[agentID].x - ahead // 2,
            ...
            color = (
                WHITE
                if self.probability_clock[agentID][Direction.LEFT.value]
                else BLACK
            )
            pygame.draw.polygon(self.display, color, arrow_points)
        ...
        if self.direction[agentID] == Direction.UP or Direction.LEFT or Direction.RIGHT:
            arrow_points = [
                (
                    self.head[agentID].x
                    + correction
                    - self.probability_clock[agentID][Direction.UP.value] // ratio,
                    self.head[agentID].y - ahead // 2,
                ),
                (
                    self.head[agentID].x + correction,
                    self.head[agentID].y
                    - self.probability_clock[agentID][Direction.UP.value]
                    - ahead // 2,
                ),
                (
                    self.head[agentID].x
            ...
            color = (
                WHITE if self.probability_clock[agentID][Direction.UP.value] else BLACK
            )
            pygame.draw.polygon(self.display, color, arrow_points)

        if (
            self.direction[agentID] == Direction.DOWN
            or Direction.LEFT
            or Direction.RIGHT
        ):
            arrow_points = [
                (
                    self.head[agentID].x
                    + correction
                    - self.probability_clock[agentID][Direction.DOWN.value - 3]
                    // ratio,
            ...
            color = (
                WHITE
                if self.probability_clock[agentID][Direction.DOWN.value - 3]
                else BLACK
            )
            pygame.draw.polygon(self.display, color, arrow_points)
    ...
    def _update_ui(self, agentID=0):
        ...
        for pt in self.snake[agentID]:
            ...
            snake_color_decay = (
                int(decay_ratio * snake_color_decay[0]),
                int(decay_ratio * snake_color_decay[1]),
                int(decay_ratio * snake_color_decay[2]),
            )
        ...
        text = font.render(
            AGENT_NAMES[agentID]
            + " Score:"
            + str(self.score[agentID])
            + " Total Reward:"
            + str(self.cumulative_reward[agentID]),
            True,
            AGENT_UI[agentID + 1][0],
        )
